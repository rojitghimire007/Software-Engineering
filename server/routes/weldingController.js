...
const postWeld = async (req, res, next) => {
  try {
    const { item, next_item, station, rp, hp, hpp, fl, wdate, fldate } =
      req.body;
    ...
    if (dupCheck.length > 0)
      throw { status: 500, message: 'This weld already exists' };
    ...
    const data = await query_resolver(connection, {
      text: 'SELECT * FROM stringing LEFT JOIN weld_seq ON stringing.item_id = weld_seq.item LEFT JOIN weld USING (weld_id) where item = $1',
      values: [item],
    });
...
const getWeld = async (req, res, next) => {
  try {
    ...
    const data = await query_resolver(
      connection,
      'SELECT * FROM stringing LEFT JOIN weld_seq ON stringing.item_id = weld_seq.item LEFT JOIN weld USING (weld_id)'
    );
    return res.status(200).json({
      success: true,
      data,
